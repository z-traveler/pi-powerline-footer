...
**Live thinking level indicator** — Appends the current thinking level to the model name (for example, `GPT-5.6 Sol:xhigh`). The standalone color-coded `thinking` segment remains available for custom layouts.
...
**Token intelligence** — Smart formatting (1.2k, 45M), used/max/percentage context display, subscription detection, configurable subscription cost display, and weekly ChatGPT Codex quota.
...
```json
{
  "powerline": {
    "preset": "default",
    "path": { "mode": "basename" },
    "model": { "display": "name", "showThinkingLevel": true },
...
| Level | Standalone display | Color |
|-------|--------------------|-------|
